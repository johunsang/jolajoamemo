...
static BROWSER_ID: Lazy<Arc<Mutex<u64>>> = Lazy::new(|| Arc::new(Mutex::new(0)));
...
/// 브라우저 초기화 (새 세션 시작)
pub async fn reset_browser() -> Result<(), String> {
    let mut browser_lock = BROWSER.lock().await;
    *browser_lock = None;
    Ok(())
}
...
pub struct ResearchTask {
    pub id: usize,
    pub task_type: String,      // "plan", "search", "crawl", "analyze", "compile"
    pub description: String,
...
pub struct ResearchState {
    ...
    pub crawled_contents: Vec<(String, String)>, // (url, content)
...
/// 웹페이지 내용 가져오기 (HTTP 기반 - 폴백용)
pub async fn fetch_page_content_http(url: &str) -> Result<String, String> {
    use tokio::time::{timeout, Duration};
    ...
    let resp = match resp_result {
        Ok(Ok(r)) => r,
        Ok(Err(e)) => return Err(format!("페이지 요청 실패: {}", e)),
        Err(_) => return Err("페이지 요청 타임아웃 (10초)".to_string()),
    };

    let text_result = timeout(Duration::from_secs(5), resp.text()).await;

    let html = match text_result {
        Ok(Ok(t)) => t,
        Ok(Err(e)) => return Err(format!("페이지 내용 읽기 실패: {}", e)),
        Err(_) => return Err("페이지 내용 읽기 타임아웃 (5초)".to_string()),
    };
...
/// 웹페이지 내용 가져오기 (간단한 HTTP - 폴백용)
pub async fn fetch_page_content_simple(url: &str) -> Result<String, String> {
    let client = Client::builder()
    ...
    let resp = client
        .get(url)
        .header("User-Agent", "Mozilla/5.0 (compatible; JolaJoaBot/1.0)")
        .send()
        .await
    ...
    let html = resp
        .text()
        .await
        .map_err(|e| format!("페이지 내용 읽기 실패: {}", e))?;
...
/// AI로 검색 결과 요약 및 리포트 생성
pub async fn generate_research_report(
    gemini_api_key: &str,
    model: &str,
    query: &str,
    search_results: &[SearchItem],
    page_contents: &[(String, String)], // (url, content)
) -> Result<ResearchReport, String> {
    let client = Client::new();
    let model = if model.is_empty() { DEFAULT_MODEL } else { model };

    // 검색 결과 요약
    let search_summary: String = search_results
        .iter()
        .enumerate()
        .map(|(i, item)| {
            format!(
                "[{}] {} ({})\n- {}\n- URL: {}",
                i + 1, item.title, item.source, item.description, item.link
            )
        })
        .collect::<Vec<_>>()
        .join("\n\n");
    ...
    // 페이지 내용 요약
    let content_summary: String = page_contents
        .iter()
    ...
    let prompt = format!(
        r#"당신은 전문 리서치 분석가입니다. 사용자의 검색 쿼리에 대해 수집된 정보를 분석하고 종합적인 리포트를 작성해주세요.
...
{{
    ...
    "full_report": "마크다운 형식의 상세 리포트 (개요, 주요 발견, 결론 섹션 포함)"
}}

요구사항:
1. 정보를 객관적으로 분석하고 사실에 기반한 리포트 작성
2. 출처가 명확한 정보 위주로 구성
3. key_points는 5개 내외로 핵심만
4. full_report는 마크다운 형식으로 구조화
5. 한국어로 작성"#,
        query, search_summary, content_summary
    );
    ...
    let json_text = extract_json(&text);
    let report_resp: ReportResponse = serde_json::from_str(&json_text)
        .map_err(|e| format!("리포트 JSON 파싱 실패: {} - 원본: {}", e, json_text))?;

    Ok(ResearchReport {
        query: query.to_string(),
        summary: report_resp.summary,
        key_points: report_resp.key_points,
        sources: search_results.to_vec(),
        full_report: report_resp.full_report,
        input_tokens,
        output_tokens,
        cost_usd: cost,
    })
...
/// 페이지에서 상호작용 가능한 요소들 추출
async fn get_page_elements(browser: &Arc<Browser>, url: &str) -> Result<String, String> {
    use tokio::time::{timeout, Duration};
    ...
    let page_result = timeout(Duration::from_secs(15), async {
        ...
        let result = page
            ...
            .await
            .map_err(|e| format!("요소 추출 실패: {}", e))?;

        let _ = page.close().await;

        Ok::<String, String>(result.into_value::<String>().unwrap_or_default())
    })
    .await;
    ...
    match page_result {
        ...
        Ok(Err(e)) => Err(e),
        Err(_) => Err("페이지 타임아웃".to_string()),
    }
}

/// 페이지 텍스트 내용 가져오기
async fn get_page_text_for_agent(browser: &Arc<Browser>, url: &str) -> Result<String, String> {
    use tokio::time::{timeout, Duration};
    ...
    match result {
        Ok(Ok(html)) => Ok(extract_text_from_html(&html).chars().take(5000).collect()),
        Ok(Err(e)) => Err(e),
...
pub async fn scan_directory(
    ...
    progress_callback(format!("📂 스캔 시작: {}", path));

    let mut all_files: Vec<FileInfo> = Vec::new();
    let mut folder_analyses: Vec<FolderAnalysis> = Vec::new();
    let mut total_folders: u64 = 0;
    let mut type_summary: HashMap<String, TypeStats> = HashMap::new();
    let mut size_map: HashMap<u64, Vec<String>> = HashMap::new(); // 중복 파일 감지용
    ...
    fn scan_recursive(
        ...
        if depth > 10 {
            return (total_size, file_count); // 너무 깊은 폴더는 스킵
        }
        ...
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    *folder_count += 1;
                    let (sub_size, sub_count) = scan_recursive(&path, files, folder_count, size_map, depth + 1);
                    total_size += sub_size;
    ...
    let (total_size, total_files) = scan_recursive(
        root_path,
        &mut all_files,
        &mut total_folders,
        &mut size_map,
    ...
    progress_callback(format!("📊 {} 파일, {} 폴더 스캔 완료", total_files, total_folders));
    ...
    let old_files: Vec<FileInfo> = all_files
        .iter()
        .filter(|f| f.modified < one_year_ago_str && f.size > 10 * 1024 * 1024) // 10MB 이상
        .cloned()
        .take(20)
        .collect();

    progress_callback("🤖 AI 분석 중...".to_string());
    ...
    // 4. 큰 파일 권고
    let very_large: Vec<&FileInfo> = large_files.iter().filter(|f| f.size > 500 * 1024 * 1024).collect(); // 500MB 이상
    if !very_large.is_empty() {
        recommendations.push(format!(
            "📁 500MB 이상 대용량 파일 {} 개 - 외장 드라이브나 클라우드 이동 권장",
            very_large.len()
    ...
    progress_callback("✅ 분석 완료!".to_string());
...
/// 텍스트에서 키/값 패턴 감지 (간단한 정규식으로 후보 추출만)
pub fn detect_secret_patterns(text: &str) -> Vec<(String, String)> {
    let mut detected: Vec<(String, String)> = Vec::new();

    // 긴 문자열 패턴만 추출 (15자 이상의 영숫자+특수문자 조합)
    let long_string_pattern = Regex::new(r"[a-zA-Z0-9_\-]{15,}").unwrap();
    for cap in long_string_pattern.find_iter(text) {
        let val = cap.as_str().to_string();
        if !detected.iter().any(|(_, v)| v == &val) {
            detected.push(("detected_value".to_string(), val));
        }
    }
...
/// AI가 텍스트를 직접 분석해서 키/값을 추출하고 분류
pub async fn analyze_text_for_secrets(
    api_key: &str,
    model: &str,
    text: &str,
) -> Result<(Vec<DetectedSecretKey>, TokenUsage), String> {
    if text.trim().is_empty() {
        return Ok((Vec::new(), TokenUsage { input_tokens: 0, output_tokens: 0, cost_usd: 0.0 }));
    }

    let today = chrono::Local::now().format("%Y-%m-%d").to_string();

    let prompt = format!(r#"다음 텍스트에서 중요한 키/값 쌍을 찾아 분류해주세요.

텍스트:
{}

key_type 분류 규칙 (반드시 따르세요):
- "API_KEY": 레이블에 "API", "api", "키", "key" 단어가 있거나, 값이 sk-, pk-, api-, key- 등으로 시작하면 무조건 API_KEY
- "TOKEN": 레이블에 "token", "토큰", "bearer" 포함
- "ACCESS_KEY": AWS 스타일 액세스 키 (AKIA로 시작)
- "PASSWORD": 레이블에 "password", "비밀번호", "pw" 포함
- "ID": 아이디, 사용자명
- "CONTACT": 전화번호, 이메일
- "SECRET": 그 외
...
JSON 배열로 응답:
- key_name: 키 이름
- key_value: 값 전체 (절대 생략하지 말 것)
- key_type: 위 규칙에 따라 분류
- provider: 서비스 이름 (MiniMax, OpenAI, AWS, Google 등)
- provider_url: 콘솔 URL (모르면 null)
- description: 한국어 설명
- issued_date: "{today}"
- expires_at: null

예시: "API 키: sk-xxx" → key_type은 반드시 "API_KEY"
...
"#, text, today = today);
    ...
    let input_tokens = response_json["usageMetadata"]["promptTokenCount"]
        .as_i64()
        .unwrap_or(0);
    ...
    let json_text = extract_json(text);
    let parsed: Vec<serde_json::Value> = serde_json::from_str(&json_text)
        .unwrap_or_default();

    let result: Vec<DetectedSecretKey> = parsed.iter()
        .filter_map(|item| {
            let key_value = item["key_value"].as_str()?.to_string();
            if key_value.len() < 10 {
                return None;
            }
            let key_name = item["key_name"].as_str().unwrap_or("SECRET").to_string();
            let mut key_type = item["key_type"].as_str().unwrap_or("SECRET").to_string();

            // AI가 틀리면 보정
            let name_lower = key_name.to_lowercase();
            let val_lower = key_value.to_lowercase();

            // 토큰/키 패턴 감지 함수
            let looks_like_api_key = |val: &str| -> bool {
                let len = val.len();

                // 1. 접두사 패턴 (sk-, pk-, api-, etc.)
                if val_lower.starts_with("sk-") || val_lower.starts_with("pk-")
                    || val_lower.starts_with("api-") || val_lower.starts_with("key-")
                    || val_lower.starts_with("api_") || val_lower.starts_with("sk_") {
                    return true;
                }

                // 2. JWT 토큰 (eyJ로 시작)
                if val.starts_with("eyJ") {
                    return true;
                }

                // 3. 긴 랜덤 문자열 (20자 이상, 영숫자+특수문자 혼합)
                if len >= 20 {
                    ...
                    let has_special = val.chars().any(|c| c == '-' || c == '_');
                    let alphanumeric_ratio = val.chars().filter(|c| c.is_alphanumeric()).count() as f32 / len as f32;

                    // 대소문자 + 숫자 혼합 또는 특수문자 포함된 긴 문자열
                    if alphanumeric_ratio > 0.8 && ((has_upper && has_lower) || (has_digit && has_special) || (has_upper && has_digit)) {
                        return true;
                    }
                }

                // 4. 32자/64자 hex 문자열 (해시/키 형식)
                if (len == 32 || len == 64) && val.chars().all(|c| c.is_ascii_hexdigit()) {
                    return true;
                }

                // 5. Base64 패턴 (길이가 4의 배수이고 Base64 문자만)
                if len >= 20 && len % 4 == 0 {
                    let is_base64 = val.chars().all(|c| c.is_ascii_alphanumeric() || c == '+' || c == '/' || c == '=');
                    if is_base64 {
                        return true;
                    }
                }
            ...
            // 값이 API 키/토큰 패턴이면 API_KEY로 분류 (최우선)
            if looks_like_api_key(&key_value) {
                key_type = "API_KEY".to_string();
            } else {
                // 값 패턴이 없을 때만 이름 기반 분류
                if name_lower.contains("api") || name_lower.contains("키") || name_lower.contains("key") {
                    key_type = "API_KEY".to_string();
                }
                // 토큰 패턴
                if name_lower.contains("token") || name_lower.contains("토큰") {
                    key_type = "TOKEN".to_string();
                }
                // 비밀번호 패턴
                if name_lower.contains("password") || name_lower.contains("비밀번호") || name_lower.contains("passwd") {
                    key_type = "PASSWORD".to_string();
                }
            }

            Some(DetectedSecretKey {
                key_name,
                key_value,
                key_type,
                provider: item["provider"].as_str().unwrap_or("Custom").to_string(),
                provider_url: item["provider_url"].as_str().map(|s| s.to_string()),
                description: item["description"].as_str().map(|s| s.to_string()),
                issued_date: item["issued_date"].as_str().unwrap_or(&today).to_string(),
                expires_at: item["expires_at"].as_str().map(|s| s.to_string()),
            })
        })
        .collect();
...
/// 감지된 키-값 쌍을 AI로 분석해서 메타데이터 추출 (값은 AI에게 보내지 않음)
pub async fn analyze_secret_keys(
    api_key: &str,
    model: &str,
    detected_pairs: &[(String, String)],
) -> Result<(Vec<DetectedSecretKey>, TokenUsage), String> {
    if detected_pairs.is_empty() {
        return Ok((Vec::new(), TokenUsage { input_tokens: 0, output_tokens: 0, cost_usd: 0.0 }));
    }

    // AI에게는 키 이름과 마스킹된 값의 패턴만 보냄
    let masked_pairs: Vec<(String, String)> = detected_pairs.iter()
        .map(|(name, value)| {
            // 값의 패턴만 추출 (실제 값은 보내지 않음)
    ...
    let prompt = format!(r#"다음은 감지된 키-값 쌍의 키 이름과 마스킹된 값 패턴입니다.
각 항목에 대해 다음 정보를 추측해주세요:

입력:
{}

key_type 분류 기준 (우선순위 순):
1. "API_KEY": 키 이름에 "API", "api", "key", "키" 포함 또는 값이 sk-, pk-, api-, key- 등으로 시작
2. "TOKEN": 키 이름에 "token", "토큰", "bearer", "jwt" 포함
3. "ACCESS_KEY": 키 이름에 "access", "secret_access" 포함 (AWS 스타일)
4. "PASSWORD": 키 이름에 "password", "비밀번호", "pw", "passwd" 포함
5. "CREDENTIAL": 키 이름에 "credential", "계정", "account" 포함
6. "ID": 키 이름에 "id", "아이디", "username", "user" 포함 (값이 짧은 경우)
7. "CONTACT": 전화번호, 이메일 형식
8. "ENV_VAR": 환경변수 형식 (ALL_CAPS_WITH_UNDERSCORE)
9. "SECRET": 그 외 비밀 값

각 항목에 대해 JSON 배열로 응답해주세요:
- key_name: 원래 키 이름 그대로
- key_type: 위 분류 기준에 따라 선택
- provider: 프로바이더 이름 (MiniMax, OpenAI, AWS, Google, GitHub, Anthropic, Slack, Custom 등)
- provider_url: 프로바이더 콘솔/대시보드 URL (알려진 경우만, 모르면 null)
- description: 간단한 설명 (한국어로, 예: "OpenAI API 키")
- issued_date: "{today}" (기본값)
- expires_at: null (알 수 없는 경우)

JSON 배열로만 응답해주세요. 다른 텍스트 없이 JSON만:
"#,
        masked_pairs.iter()
            .map(|(k, v)| format!("{}: {}", k, v))
            .collect::<Vec<_>>()
    ...
    let output_tokens = response_json["usageMetadata"]["candidatesTokenCount"]
        .as_i64()
        .unwrap_or(0);
    let model_name_for_cost = if model.is_empty() { "gemini-3-flash-preview" } else { model };
    let usage = TokenUsage {
        input_tokens,
        output_tokens,
        cost_usd: calculate_cost(model_name_for_cost, input_tokens, output_tokens),
    };
    ...
    // AI 분석 결과와 실제 값을 매칭
    let mut result: Vec<DetectedSecretKey> = Vec::new();
    for (i, item) in parsed.iter().enumerate() {
        if i >= detected_pairs.len() {
            break;
        }
        let (original_name, original_value) = &detected_pairs[i];

        result.push(DetectedSecretKey {
            key_name: item["key_name"].as_str().unwrap_or(original_name).to_string(),
            key_value: original_value.clone(),  // 실제 값 사용
            key_type: item["key_type"].as_str().unwrap_or("SECRET").to_string(),
            provider: item["provider"].as_str().unwrap_or("Custom").to_string(),
            provider_url: item["provider_url"].as_str().map(|s| s.to_string()),
            description: item["description"].as_str().map(|s| s.to_string()),
            issued_date: item["issued_date"].as_str().unwrap_or(&today).to_string(),
